...
--
-- Ported verbatim from feat/vendor-categories (commit a8c526d) -- pure SQL
-- operating only on the columns/table introduced by
-- 20260827000000_vendor_categories.sql, so it needs no adaptation for main.
--
-- FOUND BY CODE REVIEW, THEN REPRODUCED. 20260827000000 snapshots on INSERT
-- only. `PUT /api/general-ledger/[id]` can change `vendor_id` on an existing
-- row (see src/app/api/general-ledger/[id]/route.ts), and the ledger edit
-- dialog sends it, so this is reachable through the shipped UI rather than
-- being theoretical.
--
...
-- always about, so the classification must follow. 20260827000000 gets the
