...
--
-- Ported verbatim from feat/vendor-categories (commit ca78673) -- pure SQL
-- operating only on vendor_categories rows, so it needs no adaptation for
-- main.
--
--   Fixed Expense
--   Purchases            (new)
...
--   Employees            (new)
--
-- 20260827000000_vendor_categories.sql seeded only three, and used the plural
-- "Owner Withdrawals".
--
...
-- anything used it. On a freshly-migrated main (or on production, which has
-- no vendor_categories rows at all yet), this rename is a no-op for existing
-- data -- but the UPDATE form is correct regardless of that, which the delete
-- form would not be.
--
