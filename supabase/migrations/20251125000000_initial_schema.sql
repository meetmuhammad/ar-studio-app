...
-- BASELINE CORRECTION (2026-08-17), not present in the original dump.
--
-- "public"."increment_counter" (defined above) does `UPDATE counters`, and
-- scripts/seed-supabase.ts calls it by RPC to allocate order numbers, but this
-- dump never created the table. A `supabase db reset` from the uncorrected file
-- therefore produced a database that looked complete and failed the moment
-- anything allocated an order number.
--
-- Shape verified against the running local database, which does have the table:
--   id integer NOT NULL, value integer NOT NULL DEFAULT 0, PK (id),
--   RLS enabled, zero policies.
--
-- Zero policies with RLS on is deliberate, not an omission: the table is
-- reachable only by service_role (BYPASSRLS) and through increment_counter().
-- Adding a policy here would widen access beyond what production has.
--
-- Editing an already-applied baseline is safe -- `db push` skips versions
-- recorded as applied and never compares content. This changes only what a
-- from-zero replay produces locally, which is the entire point.
...
-- BASELINE CORRECTION (2026-08-17), continued.
--
-- The table is not enough: increment_counter() is
--   UPDATE counters SET value = value + 1 WHERE id = counter_id RETURNING value
-- so with no row for that id it updates nothing and returns NULL *without
-- raising*. scripts/seed-supabase.ts checks only rpcError, never the returned
-- value, so a from-zero database seeds orders with null order numbers instead
-- of failing loudly.
--
-- Counter 1 is the order-number sequence. Seeded at 0 so the first allocation
-- returns 1. Idempotent, and it cannot disturb a database that already has the
-- row -- which is every deployed environment, none of which will ever execute
-- this file.
